...
public class PrayerTimes {
    ...
    public String getCurrentName(Context context) {
        return getName(context, c, current);
    }

    public String getNextName(Context context) {
        return getName(context, n, next);
    }
    ...
    public static String formatPrayerTime(GregorianCalendar cal, int round) {
        SimpleDateFormat sdf = new SimpleDateFormat(round != 0 ? "HH:mm" : "HH:mm:ss", Locale.getDefault());
    ...
    private String formatTimeInterval(Context context, GregorianCalendar from, GregorianCalendar to) {
        ...
        Timber.d(formatPrayerTime(to) + "-" + formatPrayerTime(from) + "=" + hours + ":" + minutes + (rounded ? "" : ":" + seconds));

        return hours != 0 ? rounded ? String.format(Locale.getDefault(), context.getString(R.string.time_interval_rounded), hours, minutes) : String.format(Locale.getDefault(), context.getString(R.string.time_interval), hours, minutes, seconds) : rounded ? String.format(Locale.getDefault(), context.getString(R.string.time_interval_0h_rounded), minutes) : String.format(Locale.getDefault(), context.getString(R.string.time_interval_0h), minutes, seconds);
    }
