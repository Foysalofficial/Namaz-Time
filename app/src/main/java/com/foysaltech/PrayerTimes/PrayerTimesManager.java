package com.foysaltech.PrayerTimes;
...
import com.foysaltech.PrayerTimes.receivers.AlarmReceiver;
import com.foysaltech.PrayerTimes.receivers.BootAndTimeChangeReceiver;
...
public class PrayerTimesManager {
    ...
    public static String formatTimeToNextPrayer(Context context, GregorianCalendar from) {
        ...
        return context.getString(R.string.time_to_next) + " " + sPrayerTimes.formatTimeToNextPrayer(context, from);
    }
    ...
    public static String formatTimeFromCurrentPrayer(Context context, GregorianCalendar to) {
        ...
        return String.format(context.getString(R.string.time_up), sPrayerTimes.getCurrentName(context)) + sPrayerTimes.formatTimeFromCurrentPrayer(context, to);
    }
    ...
    private static void calcPrayerTimes(Context context, GregorianCalendar nowCal, City city) {
        ...
        PTLocation location = new PTLocation(city.getLatitude(), city.getLongitude(), gmtDiffHrs, dst, city.getAltitude(), 1010 /* pressure */, 10 /* temperature */);
        ...
        for (i = 0; i < Prayer.NB_PRAYERS; i++) {
            ...
            Timber.d(PrayerTimes.getName(context, i, nowCal) + " " + PrayerTimes.formatPrayerTime(ptCal[i], sMethod.round));
        }
